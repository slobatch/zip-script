...
File.open("outputs/reg_cust#{low_s}-#{high_s}.csv.tmp", 'w') do |file|
  CSV.foreach('inputs/reg_cust.csv', headers:true) do |row|
  # CSV.foreach('inputs/reg_cust.csv') do |row|
    if $. >= low && $. <= high
      begin
        ...
        if zip_status
          puts "Reg #{$.} Zip Status: #{zip_status}"
          row["zip_check"]="#{zip_status}"
          file.write(row)
        else
          puts "Reg #{$.} Zip Status: null"
          row["zip_check"]="null"
          file.write(row)
        end
        # sleep(0.015)
      rescue
        puts "Reg #{$.} Zip Status: N/A"
